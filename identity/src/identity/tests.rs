...
fn from_mnemonic_is_deterministic() {
    ...
    assert_eq!(identity.encryption_public_key(), again.encryption_public_key());
...
fn sign_and_verify_round_trip() {
    ...
    assert!(crate::verify(&identity.signing_public_key(), b"hello", &sig));
    assert!(!crate::verify(&identity.signing_public_key(), b"tampered", &sig));
}
