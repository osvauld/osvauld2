...
/// gap-log 1.6's guard. The definitions file teaches an editor what `ui`, `doc` and `gfx` are;
/// this is what stops it becoming a lie the first time someone adds a prop.
///
/// It fails rather than silently regenerating, because a test that rewrites tracked files on every
/// run makes `git status` meaningless. `BLESS=1` writes it.
#[test]
fn generated_defs_are_current() {
    let want = crate::defs::render().expect("render definitions");
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("..")
        .join(crate::defs::DEFS_PATH);

    if std::env::var_os("BLESS").is_some() {
        std::fs::create_dir_all(path.parent().unwrap()).expect("create lua-types/");
        std::fs::write(&path, &want).expect("write definitions");
        return;
    }
    // Report the first differing line, not `assert_eq!` on two whole files: the definitions are
    // ~160 lines and dumping both twice buries the one line that matters.
    let have = std::fs::read_to_string(&path).unwrap_or_default();
    if have != want {
        let (h, w): (Vec<_>, Vec<_>) = (have.lines().collect(), want.lines().collect());
        let at = h
            .iter()
            .zip(&w)
            .position(|(a, b)| a != b)
            .unwrap_or(h.len().min(w.len()));
        panic!(
            "{} is stale at line {}:\n  have: {:?}\n  want: {:?}\nRegenerate:\n    \
             BLESS=1 cargo test -p app_host generated_defs_are_current",
            crate::defs::DEFS_PATH,
            at + 1,
            h.get(at).copied().unwrap_or("<end of file>"),
