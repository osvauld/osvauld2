...
const INTER: &[u8] = include_bytes!("../../sthalam/assets/fonts/Inter-Regular.ttf");
const INTER_SB: &[u8] = include_bytes!("../../sthalam/assets/fonts/Inter-SemiBold.ttf");
const JBMONO: &[u8] = include_bytes!("../../sthalam/assets/fonts/JetBrainsMono-Regular.ttf");

/// A Context whose font system matches the embedded faces: Inter (proportional), JetBrains Mono
/// (monospace), Inter SemiBold under the editor's bold family.
fn ctx_with_fonts() -> egui::Context {
    ...
    fonts
        ...
        .or_default()
        .extend(["inter_sb".to_owned(), "inter".to_owned()]);
    ctx.set_fonts(fonts);
    ctx
}
...
fn exports_a_valid_multipage_pdf() {
    ...
    let _ = ctx.run(raw, |ctx| {
        egui::CentralPanel::default().show(ctx, |ui| {
            let fonts = PdfFonts { regular: INTER, bold: INTER_SB, mono: JBMONO };
