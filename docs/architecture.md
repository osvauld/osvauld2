...
| `courier` | transport-independent node/session protocol. Today it issues and verifies signed bootstrap tickets, desktop/node relationship permits, first-admin claim, and admin reconnect as pure message transitions; workspace publish/sync and transport adapters remain unbuilt. |
