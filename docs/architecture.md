...
| `kunki` | the sovereign-node binary. Today it only creates/loads a passphrase-sealed node identity and prints a `courier` bootstrap connection ticket; workspace publish, sync, admin storage, and QUIC protocol handling remain unbuilt. |
